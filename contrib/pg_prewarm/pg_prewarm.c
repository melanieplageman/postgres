...
Datum slow_consumer(PG_FUNCTION_ARGS)
{
	...
	pgsr = pg_streaming_read_alloc(0, (uintptr_t) &p,
									prewarm_buffer_next,
									prewarm_buffer_release, consumptions);

	dev_log = palloc(sizeof(PgStreamingReadDevLog));
	dev_log->completion_log = aio_dev_make_completion_log(nblocks);
	dev_log->wait_log = aio_dev_make_wait_log(nblocks);
	dev_log->consumption_log = aio_dev_make_consumption_log(nblocks);
	...
	pg_streaming_read_write_log(pgsr);
	pg_streaming_read_free(pgsr);
	relation_close(rel, AccessShareLock);
