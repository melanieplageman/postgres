...
sub init_from_backup
{
	...
	if (defined $params{combine_with_prior})
	{
		...
		push @combineargs, @prior_backup_path, $backup_path,
		  '--output' => $data_path;
		$self->_run_pg_combinebackup_with_debug_log(@combineargs);
	}
...
{
	my ($self, @cmd) = @_;
	my $debug_log = $self->basedir . '/pg_combinebackup_debug.log';
	...
	# pg_combinebackup succeeded. Retain its debug log so that, if a later TAP
	# assertion in this test fails, the END block can print the combine step
	# that produced the restored state; on a clean run the END block removes it.
	# On failure above we already printed the log and bailed out, so it is
	# deliberately not added to the list here (which also avoids printing it
	# twice).
