...
PG_FUNCTION_INFO_V1(prepare_rel_blocks_for_bgwriter);
Datum
prepare_rel_blocks_for_bgwriter(PG_FUNCTION_ARGS)
{
	Oid			relid = PG_GETARG_OID(0);
	ArrayType  *blocksarray = PG_GETARG_ARRAYTYPE_P(1);
	Relation	rel;
	...
	if (RelationUsesLocalBuffers(rel))
		ereport(ERROR,
				errmsg("cannot prepare local buffers for bgwriter"));
	...
	for (int i = 0; i < nblocks; i++)
	{
		...
		/*
		 * There's no way to explicitly guarantee the usage count stays 0 for our
		 * bgwriter tests. It requires careful coding and setup in the test to
		 * make sure that nothing else is going to look for a free buffer.
		 */
...
eager_clean_rel_block(PG_FUNCTION_ARGS)
{
	...
	EagerCleanBufferForTesting(buf);
