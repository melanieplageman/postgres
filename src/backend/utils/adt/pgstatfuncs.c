...
pg_stat_get_vm_vis(PG_FUNCTION_ARGS)
{
	...
	PG_RETURN_INT64(tabentry->vm_set.vm_vis);
}
...
pg_stat_get_vm_unvis(PG_FUNCTION_ARGS)
{
	...
	PG_RETURN_INT64(tabentry->vm_unset.vm_unvis);
}
...
Datum
pg_stat_get_vm_insert_unfreezes(PG_FUNCTION_ARGS)
{
	Oid			relid = PG_GETARG_OID(0);
	PgStat_StatTabEntry *tabentry;
	...
	PG_RETURN_INT64(tabentry->vm_unset.vm_unfreezes[PGSTAT_UNSET_INSERT]);
}

Datum
pg_stat_get_vm_mod_unfreezes(PG_FUNCTION_ARGS)
{
	Oid			relid = PG_GETARG_OID(0);
	PgStat_StatTabEntry *tabentry;
	...
	PG_RETURN_INT64(tabentry->vm_unset.vm_unfreezes[PGSTAT_UNSET_MOD]);
}
