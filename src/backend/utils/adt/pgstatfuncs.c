...
pg_stat_lsntime_stream(PG_FUNCTION_ARGS)
{
	...
	for (size_t i = 0; i < stream->length; i++)
	{
		...
		values[0] = LSNGetDatum(stream->data[i].lsn);
		values[1] = TimestampTzGetDatum(stream->data[i].time);
		tuplestore_putvalues(rsinfo->setResult, rsinfo->setDesc,
							 values, nulls);
	}
 ...
 * data, the lower bound will be NULL. If it is in the future, the upper bound
 * will be NULL.
 */
Datum
pg_stat_lsn_bounds_for_time(PG_FUNCTION_ARGS)
{
	...
	values[0] = LSNGetDatum(lower.lsn);
	values[1] = LSNGetDatum(upper.lsn);
 ...
 * have data, the lower bound will be NULL. If the passed-in LSN is in
 * the future, the upper bound will be NULL.
 */
Datum
pg_stat_time_bounds_for_lsn(PG_FUNCTION_ARGS)
{
	...
	values[1] = TimestampTzGetDatum(upper.time);
