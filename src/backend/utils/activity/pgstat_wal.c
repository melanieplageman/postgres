 ...
 * covering the target_time. If target_time is older than the oldest value on
 * the stream, lower will be zeroed out. If target_time is newer than the
 * newest value on the stream, upper will be zeroed out. If the stream is
 * empty, both upper and lower will be zeroed out. The caller should always
 * check if upper and/or lower's LSN member is InvalidXLogRecPtr to detect
 * this.
 */
static void
					...
					LSNTime *lower, LSNTime *upper)
{
	 ...
	 */
	if (lower->lsn != InvalidXLogRecPtr && upper->lsn != InvalidXLogRecPtr)
		return;
	...
	if (upper->lsn == InvalidXLogRecPtr && target_time <= current_time)
						  ...
						  LSNTime *upper)
{
	...
	*upper = LSNTIME_INIT(InvalidXLogRecPtr, 0);
	...
	{
		elog(DEBUG1,
			 "Attempt to identify time bounds for LSN: \"%" PRIu64 "\" using empty LSNTimeStream.",
			 target_lsn);
		return;
	}
					...
					LSNTime *lower, LSNTime *upper)
{
	...
	if (lower->lsn == InvalidXLogRecPtr && target_lsn >= current_lsn)
		*lower = LSNTIME_INIT(current_lsn, current_time);
