...
static void lsntime_insert(LSNTimeStream *timeline, TimestampTz time, XLogRecPtr lsn);

XLogRecPtr	estimate_lsn_at_time(const LSNTimeStream *timeline, TimestampTz time);
TimestampTz estimate_time_at_lsn(const LSNTimeStream *timeline, XLogRecPtr lsn);
...
static int
lsntime_merge_target(LSNTimeStream *timeline)
{
	/* Don't merge if free space available */
	Assert(timeline->length == LSNTIMESTREAM_VOLUME);
 ...
 */
void
lsntime_insert(LSNTimeStream *timeline, TimestampTz time,
			   XLogRecPtr lsn)
{
	int			merge_target;
	LSNTime		entrant = {.lsn = lsn,.time = time,.members = 1};

	if (timeline->length < LSNTIMESTREAM_VOLUME)
	{
		/*
		...
		Assert(timeline->length == 0 ||
			   (lsn >= timeline->data[LSNTIMESTREAM_VOLUME - timeline->length].lsn &&
				time >= timeline->data[LSNTIMESTREAM_VOLUME - timeline->length].time));
		 ...
		 */
		timeline->length++;
		timeline->data[LSNTIMESTREAM_VOLUME - timeline->length] = entrant;
		return;
	}
	...
	if (merge_target < 0)
	{
		elog(WARNING, "Unable to insert LSNTime to LSNTimeStream. Merge failed. Out of space");
		return;
	}
 ...
 */
XLogRecPtr
estimate_lsn_at_time(const LSNTimeStream *timeline, TimestampTz time)
{
	XLogRecPtr	result;
	int64		time_elapsed,
				lsns_elapsed;
	 ...
	 * end point.
	 */
	for (int i = LSNTIMESTREAM_VOLUME - timeline->length; i < LSNTIMESTREAM_VOLUME; i++)
	{
 ...
 */
TimestampTz
estimate_time_at_lsn(const LSNTimeStream *timeline, XLogRecPtr lsn)
{
	int64		time_elapsed,
				lsns_elapsed;
	TimestampTz result;
...
void
pgstat_wal_update_lsn_time_stream(TimestampTz time, XLogRecPtr lsn)
{
	PgStatShared_Wal *stats_shmem = &pgStatLocal.shmem->wal;
...
pg_lsntimeline(PG_FUNCTION_ARGS)
{
	...
	LSNTimeStream *timeline = &stats->timeline;
