 ...
 * early vm_unsets (pages' all-visible durations shorter than
 * target_freeze_duration). See:
 *
...
/*
 * Calculate the "age" (duration the page was unmodified) of the most recently
 * modified page we will consider freezing while vacuuming this table. We use
 * the table-level stats about the all-visible durations of pages in this table
 * to calculate this threshold.
 *
 * Returns InvalidXLogRecPtr if we should freeze all pages that meet any other
 * eager freeze criteria.
 */
LSNInterval
pgstat_min_freezable_page_age(Oid tableoid, bool shared)
{
	 ...
	 * standard deviation).
	 */
	accumulator_calculate(&tabentry->vm_unsets, &mean, &stddev);

	/*
	 * Calculate the age of the youngest page that should be eagerly frozen.
	 *
	 * We'll eagerly freeze a page if the probability that it will be modified
	 * before target_freeze_duration is less than approximately 5%. This
	 * threshold occurs when the cumulative distribution function of the early
	 * vm_unsets exceeds 95%. We assume that if a page has survived past the
	 * age when 95% of early vm_unsets have occurred for this table, then it's
	 * safe to freeze.
	 *
	 ...
	 * side of freezing by setting it to InvalidXLogRecPtr.
	 */
 ...
 * target_freeze_duration. Record both the page's all-visible duration and the
 * page's former status in the VM. The distribution of all-visible durations
 * can be used to predict whether or not a given page is likely to remain
 * unmodified for longer than target_freeze_duration.
 *
					  ...
					  XLogRecPtr current_lsn, uint8 old_vmbits)
{
	 ...
	 * as an early "vm_unset" for vacuum's adaptive freeze statistics. We want
	 * to determine the likelihood that a page being vacuumed will be modified
	 * before target_freeze_duration has elapsed, irrespective of whether or
	 * not we got it right last vacuum, so we count all-visible pages even if
	 * they weren't frozen.
	 *
	 * If lsn_target_frz_dur is invalid, we don't have enough data to estimate
	 * the LSN duration corresponding to target_freeze_duration. In this case,
	 * all pages are considered to be modified sooner than
	 ...
	 */
	if (wal_stats->lsn_target_frz_dur == InvalidXLogRecPtr ||
		vm_duration_lsns < wal_stats->lsn_target_frz_dur)
 ...
 * modifying pages before target_freeze_duration to the corresponding shared
 * stats.
 */
static void
