...
/*
 * The max number of page age values we want to represent in the accumulator at
 * a given time. Once we reach max capacity, we remove values before inserting
 * new ones. This allows the accumulator to more accurately reflect the table
 * as the access pattern changes over time.
 */
#define UNSET_ACCUMULATOR_CAPACITY 10000

/*
 * Calculate the age of the most recently modified page we will consider
 * freezing during vacuum.
 */
XLogRecPtr
 ...
 * this page, so the page's age at insert is unrelated to the page modification
 * pattern.
 */
void
					  ...
					  XLogRecPtr current_lsn, uint8 old_vmbits)
{
	...
	/*
	 * If the page is being modified before target_freeze_duration, count it as
	 * an unset for vacuum freeze statistics. We want to determine the
	 ...
	 * UNSET_ACCUMULATOR_CAPACITY because we will handle that before flushing
	 * this backends accumulated values to the shared memory version.
	 */
 ...
 * new target PgStat_VMUnset would represent more than UNSET_ACCUMULATOR_CAPACITY
 * values, remove enough values to fall below UNSET_ACCUMULATOR_CAPACITY.
 */
static inline void
pgstat_unset_absorb(PgStat_VMUnset *target, PgStat_VMUnset *source)
{
	...
	if (new_total_unsets > UNSET_ACCUMULATOR_CAPACITY)
		n_remove = new_total_unsets - UNSET_ACCUMULATOR_CAPACITY;

	/*
	 * To ensure the accumulator stays current with the table's access pattern,
	 * remove values in excess of UNSET_ACCUMULATOR_CAPACITY before absorbing
	 * in new values.
	 */
	if (n_remove > 0)
		accumulator_remove(&target->early_unsets,
				UNSET_ACCUMULATOR_CAPACITY);
