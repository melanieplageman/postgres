				...
				OffsetNumber *off_loc)
{
	...
	if (!do_prune)
	{
		...
		if (!do_freeze && hint_bit_fpi)
			do_freeze = pagefrz && (pagefrz->freeze_required || whole_page_freezable);
				 ...
				 PruneState *prstate, PruneResult *presult)
{
	...
	for (;;)
	{
		...
		if (ItemIdIsDead(lp))
		{
			 ...
			 * was already marked dead. If it will not be marked LP_UNUSED, it
			 * will remain LP_DEAD, making the page not all_visible.
			 */
