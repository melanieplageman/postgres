				...
				PruneResult *presult)
{
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		*off_loc = offnum;
		 ...
		 * heap_prune_chain() did not mark it to be set dead, redirected, or
		 * unused, it will stay LP_NORMAL. This makes it unsafe to truncate
		 * the relation. LP_REDIRECT items also make the relation unsafe for
		 * truncation, but that is handled in heap_prune_record_redirect().
		 *
