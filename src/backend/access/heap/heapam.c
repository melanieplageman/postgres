				  ...
				  HeapPageFreeze *pagefrz)
{
	...
	*flags = 0;
	...
	/* Not on-access freezing multixacts currently */
	Assert(!pagefrz->opportunistic);
						  ...
						  bool opportunistic)
{
	...
	{
		/*
		 * Raw xmax is a MultiXactId. Currently we do not attempt to
		 * opportunistically freeze MultiXacts.
		 */
		...
		if (!opportunistic)
		{
			...
			/*
			 * We will either remove xmax completely (in the "freeze_xmax"
			 * path), process xmax by replacing it (in the "replace_xmax"
			 * path), or perform no-op xmax processing.  The only constraint
			 * is that the FreezeLimit/MultiXactCutoff postcondition must
			 * never be violated.
			 */
			...
			{
				/*
				 * xmax is a MultiXactId, and nothing about it changes for
				 * now. This is the only case where 'freeze_required' won't
				 * have been set for us by FreezeMultiXactId, as well as the
				 * only case where neither freeze_xmax nor replace_xmax are
				 * set (given a multi).
				 *
				 * This is a no-op, but the call to FreezeMultiXactId might
				 * have ratcheted back NewRelfrozenXid and/or NewRelminMxid
				 * trackers for us (the "freeze page" variants, specifically).
				 * That'll make it safe for our caller to freeze the page
				 * later on, while leaving this particular xmax undisturbed.
				 *
				 * FreezeMultiXactId is _not_ responsible for the "no freeze"
				 * NewRelfrozenXid/NewRelminMxid trackers, though -- that's
				 * our job.  A call to heap_tuple_should_freeze for this same
				 * tuple will take place below if 'freeze_required' isn't set
				 * already. (This repeats work from FreezeMultiXactId, but
				 * allows "no freeze" tracker maintenance to happen in only
				 * one place.)
				 */
			...
			{
				/*
				 * xmax will become an updater Xid (original MultiXact's
				 * updater member Xid will be carried forward as a simple Xid
				 * in Xmax).
				 */
				if (vistest)
				 ...
				 * we know the return Xid is a tuple updater (i.e. not merely
				 * a locker.) Also note that the only reason we don't
				 * explicitly worry about HEAP_KEYS_UPDATED is because it
				 * lives in t_infomask2 rather than t_infomask.
				 */
			...
			else if (flags & FRM_RETURN_IS_MULTI)
			{
				...
				/*
				 * xmax is an old MultiXactId that we have to replace with a
				 * new MultiXactId, to carry forward two or more original
				 * member XIDs.
				 */
				Assert(!MultiXactIdPrecedes(newxmax, pagefrz->cutoffs->OldestMxact));

				/*
				 * We can't use GetMultiXactIdHintBits directly on the new
				 * multi here; that routine initializes the masks to all
				 * zeroes, which would lose other bits we need.  Doing it this
				 * way ensures all unrelated bits remain untouched.
				 */
				frz->t_infomask &= ~HEAP_XMAX_BITS;
			...
			{
				/*
				 * Freeze plan for tuple "freezes xmax" in the strictest
				 * sense: it'll leave nothing in xmax (neither an Xid nor a
				 * MultiXactId).
				 */
				  ...
				  TransactionId FreezeLimit, TransactionId MultiXactCutoff)
{
	...
	do_freeze = heap_prepare_freeze_tuple(tuple, &pagefrz, &frz,
			&totally_frozen, NULL, false);
						 ...
						 bool opportunistic, GlobalVisState *vistest)
{
	...
	if (TransactionIdIsNormal(xid))
	{
		...
		if (opportunistic)
			lazy_update_freeze_limit(cutoffs, vistest);
