							  ...
							  uint64 *lossy_pages, uint64 *exact_pages)
{
	...
	*recheck = tbmres.recheck;
	 ...
	 */
	if (scan->pstate == NULL &&
		!tbm_exhausted(&scan->prefetch_iterator) &&
		bscan->pfblockno < block)
		elog(ERROR, "prefetch and main iterators are out of sync. pfblockno: %d. block: %d", bscan->pfblockno, block);
							  ...
							  uint64 *exact_pages)
{
	...
	else if (pstate->prefetch_target < bscan->prefetch_maximum)
	{
		/* take spinlock while updating shared state */
		SpinLockAcquire(&pstate->mutex);
		if (pstate->prefetch_target < bscan->prefetch_maximum)
	...
	/* TODO: why is this outside of the ifdef */
...
BitmapAdjustPrefetchTarget(BitmapHeapScanDesc *bscan)
{
	...
	if (pstate == NULL)
	{
		...
		else if (bscan->prefetch_target >= bscan->prefetch_maximum / 2)
			bscan->prefetch_target = bscan->prefetch_maximum;
		else if (bscan->prefetch_target > 0)
			bscan->prefetch_target *= 2;
		else
			bscan->prefetch_target++;
		return;
	}
	...
	if (pstate->prefetch_target < bscan->prefetch_maximum)
	{
		...
		if (pstate->prefetch_target >= bscan->prefetch_maximum)
			 /* don't increase any further */ ;
		else if (pstate->prefetch_target >= bscan->prefetch_maximum / 2)
			pstate->prefetch_target = bscan->prefetch_maximum;
