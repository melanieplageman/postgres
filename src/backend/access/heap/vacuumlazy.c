...
lazy_scan_heap(LVRelState *vacrel)
{
	...
	for (; blkno < rel_pages; blkno = next_blkno)
	{
		...
		if (!(vmbits & VISIBILITYMAP_ALL_VISIBLE) && presult.all_visible)
		{
		 ...
		 */
		else if ((vmbits & VISIBILITYMAP_ALL_VISIBLE) && presult.all_visible &&
				 presult.all_frozen && !(vmbits & VISIBILITYMAP_ALL_FROZEN) &&
 ...
 *
 * Returns whether or not pruning managed to free space on the page.
 */
static bool
				...
				PruneResult * presult)
{
	...
	if (lpdead_items > 0)
	{
		...
		/* MTODO: figure out newly lp dead/already lp dead stuff */
