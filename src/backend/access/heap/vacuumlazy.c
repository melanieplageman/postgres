...
typedef struct LVRelState
{
	...
	LocalAVPages av_pages[NUM_LOCAL_AV_PAGES];
				...
				BufferAccessStrategy bstrategy)
{
	...
	pgstat_relation_start_av_interval(rel, vacrel->av_pages, insert_lsn, starttime);
	...
	pgstat_relation_start_av_interval(rel, vacrel->av_pages, insert_lsn, endtime);
...
lazy_scan_heap(LVRelState *vacrel)
{
	...
	while (heap_vac_scan_next_block(vacrel, &blkno, &all_visible_according_to_vm))
	{
		...
		if (vacrel->scanned_pages % VACUUM_AV_EVERY_PAGES == 0)
			pgstat_relation_old_av_pages(vacrel->rel);
	...
	*has_lpdead_items = (presult.lpdead_items > 0);
	...
	if (!all_visible_according_to_vm && presult.all_visible)
	{
		...
		if (presult.all_frozen)
		{
			...
			pgstat_relation_vacuum_count_av(vacrel->rel,
					PageGetLSN(page),
					vacrel->av_pages);
	...
	else if (presult.lpdead_items > 0 && PageIsAllVisible(page))
	{
		...
		if ((old_vmbits & VISIBILITYMAP_ALL_VISIBLE) &&
			...
			pgstat_relation_vacuum_count_un_av(vacrel->rel, og_page_lsn);
	}
					  ...
					  Buffer vmbuffer)
{
								 ...
								 &all_frozen))
	{
		...
		if ((old_vmbits & VISIBILITYMAP_ALL_VISIBLE) &&
			...
			pgstat_relation_count_un_av(vacrel->rel, old_page_lsn);
	}
