		...
		vacrel->next_unskippable_block == InvalidBlockNumber)
	{
		...
		if (vacrel->aggressive == VAC_SEMIAGGRESSIVE)
		{
					 ...
					 MAX_SUCCESSIVE_EAGER_SCAN_FAILS)
			{
				 ...
				 * likely to have similar age data, we will start eager
				 * scanning again some time later. For a small table, we'll
				 * retry eager scanning every quarter of the table. For a
				 * larger table, we'll consider eager scanning again after
				 * processing another segment's worth of data.
				 *
