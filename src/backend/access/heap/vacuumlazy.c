 ...
 * standard deviation of the all-visible duration of pages.
 */
...
lazy_scan_heap(LVRelState *vacrel)
{
	...
	while (heap_vac_scan_next_block(vacrel, &blkno, &all_visible_according_to_vm))
	{
		...
		if (vacrel->scanned_pages <= 1 ||
			vacrel->scanned_pages % REFRESH_UNFREEZE_STATS_EVERY_PAGES == 0)
		{
			PgStat_WalStats *wal_stats = pgstat_fetch_stat_wal();

			vacrel->cutoffs.lsn_target_freeze_duration = wal_stats->lsn_target_freeze_duration;
			pgstat_tab_unfreeze_distribution(RelationGetRelid(vacrel->rel),
											 vacrel->rel->rd_rel->relisshared,
											 &vacrel->cutoffs.early_unfreezes);
		}
