...
XLogPrefetcherReadRecord(XLogPrefetcher *prefetcher, char **errmsg)
{
	...
	if (unlikely(XLogPrefetchReconfigureCount != prefetcher->reconfigure_count))
	{
		...
		/*
		 * We need to use the 'extended' version of the streaming read
		 * callback, because we need to be able to read from multiple
		 * relations and control the mode.
		 */
		prefetcher->streaming_read =
			pg_streaming_read_buffer_alloc_ext(max_ios,
											   0,
