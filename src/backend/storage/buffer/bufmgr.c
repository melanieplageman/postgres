...
PrefetchSharedBufferWithoutRelcache(SMgrRelation smgr_reln,
									ForkNumber forkNum,
									BlockNumber blockNum,
									char relpersistence)
{
	...
	/*
	 * Report the buffer it was in at that time.  The caller may be able to
	 * avoid a buffer table lookup, but it's not pinned and it must be
	 * rechecked!
	 */
				 ...
				 IOContext *io_context)
{
	...
	*hit = false;
	...
	else if (mode == RBM_ZERO_AND_LOCK || mode == RBM_ZERO_AND_CLEANUP_LOCK)
	{
		 ...
		 * buffer content lock before marking the page as valid, to make sure
		 * that no other backend sees the zeroed page before the caller has
					 ...
					 PgStreamingWrite *pgsw)
{
	...
	/*
	 * If there are pre-existing IOs in-flight, we can't block on the content
	 * lock, it could lead to a deadlock. So first wait for outstanding IO,
	 * and then block on acquiring the lock.
	 */
	if (pg_streaming_write_inflight(pgsw) > 0 &&
		LWLockConditionalAcquire(content_lock, LW_SHARED))
	{
			...
			IOContext io_context)
{
	...
	if (!io_data_force_async)
	{
		 ...
		 * and end the BM_IO_IN_PROGRESS state.
		 */
				 ...
				 IOContext io_context)
{
	 ...
	 * process would need to ensure the IO is completed before being allowed
	 * to sleep.
	 *
