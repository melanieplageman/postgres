...
static uint32 CurrentMaxEagerWriteBatchSize(BufferAccessStrategy strategy);
...
static BlockNumber InitCenteredEagerWriteBatch(BufferDesc *required_bufdesc,
											   BufferWriteBatch *batch,
...
static uint32
CurrentMaxEagerWriteBatchSize(BufferAccessStrategy strategy)
{
	uint32		limit;

	limit = MaxWriteBatchSize(strategy);
	limit = Min(eager_clean_max_batch_size, limit);
...
again:
	...
	count_eviction = buf_state & BM_VALID;
										 ...
										 IOContext io_context))
{
	...
	/*
	 * For pg_stat_io, buffers reused from a strategy ring are IOOP_REUSE, and
	 * all other selected buffers are IOOP_EVICT.
	 */
	*io_op = IOOP_EVICT;
...
EagerCleanBuffer(BufferAccessStrategy strategy, BufferDesc *buf_hdr, IOContext io_context)
{
	...
	if ((status = StartBufferIO(buf, false, true, NULL)) !=
						 ...
						 IOContext io_context)
{
	...
	for (;;)
	{
		...
		if (next_bufdesc)
		{
			 ...
			 * current batch ends.
			 */
			limit = InitForwardEagerWriteBatch(strategy, next_bufdesc, &batch);
			if (limit > 1)
	...
	while (!binaryheap_empty(ts_heap))
	{
		...
		while (batch.n < limit)
		{
			...
			if (!BlockNumberIsValid(batch_start))
			{
				...
				limit = smgrmaxcombine(batch.reln, batch.forkno, batch_start);
...
InitForwardEagerWriteBatch(BufferAccessStrategy strategy,
						   BufferDesc *required_bufdesc, BufferWriteBatch *batch)
{
	BlockNumber limit;
	...
	batch->max_lsn =
		pg_atomic_read_u64(&required_bufdesc->state) & BM_PERMANENT ?
		BufferGetLSN(required_bufdesc) : InvalidXLogRecPtr;

	limit = CurrentMaxEagerWriteBatchSize(strategy);
	limit = Min(limit, smgrmaxcombine(batch->reln, batch->forkno,
									  required_bufdesc->tag.blockNum));
	Assert(BlockNumberIsValid(required_bufdesc->tag.blockNum));
 ...
 */
static BlockNumber
InitCenteredEagerWriteBatch(BufferDesc *required_bufdesc,
							BufferWriteBatch *batch,
							BlockNumber *scan_start, BlockNumber *scan_end)
{
	...
	max_batch_size = CurrentMaxEagerWriteBatchSize(NULL);
								 ...
								 BufferWriteBatch *batch)
{
	...
	max_batch_size = InitCenteredEagerWriteBatch(required_bufdesc,
												 batch,
												 &scan_start, &scan_end);
	if (max_batch_size <= 1)
	...
	/*
	 * Loop backwards from our required block, looking for up to
	 * max_batch_size contiguous blocks, but stop once we hit the start of the
	 ...
	 * find, up to max_batch_size, in our bufdescs array. Stop if we hit the
	 * end of the segment.
	 */
								...
								bool *usage_count_zero)
{
	...
	*usage_count_zero =
		BUF_STATE_GET_USAGECOUNT(pg_atomic_read_u64(&bufdesc->state)) == 0;
				 ...
				 IOContext io_context)
{
	...
	errcallback.arg = batch->bufdescs[0];
	error_context_stack = &errcallback;
	...
	for (uint32 i = 0; i < batch->n; i++)
	{
		...
		Assert(!(pg_atomic_read_u64(&bufdesc->state) & BM_PERMANENT) ||
			   !XLogNeedsFlush(BufferGetLSN(bufdesc)));
		Assert(BufTagGetForkNum(&bufdesc->tag) == batch->forkno);
		Assert(bufdesc->tag.blockNum == batch_start + i);
	}
#endif
