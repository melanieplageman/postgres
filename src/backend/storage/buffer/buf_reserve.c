	...
	while (true)
	{
		...
		if (bbr.clean_clean_size > 0)
		{
			...
			if (TryReuseBuffer(entry->buffer, io_context))
			{
				 ...
				 * shared buffers are counted as IOOP_EVICT in the
				 * corresponding context (e.g. IOCONTEXT_BULKWRITE). Shared
				 * buffers are evicted by a strategy in two cases: 1) while
				 * initially claiming buffers for the strategy ring 2) to
				 * replace an existing strategy ring buffer because it is
				 * pinned or in use and cannot be reused.
				 *
				 * Blocks evicted from buffers already in the strategy ring
				 * are counted as IOOP_REUSE in the corresponding strategy
				 * context.
				 *
				 * At this point, we can accurately count evictions and
				 * reuses, because we have successfully claimed the valid
				 * buffer. Previously, we may have been forced to release the
				 * buffer due to concurrent pinners or erroring out.
				 *
...
BufReserveInit(void)
{
	...
	/*
	 * Don't allow buffers pinned to take more than 1/8 of s_b, otherwise we
	 * run into danger of running out of buffers.
	 */
	bbr.free_max_size = Min(Max(buf_per_backend / 8, 1), 8);
	bbr.free_target_size = bbr.free_max_size;
/* XXX:make smarter */
	...
	bbr.clean_target_size = bbr.clean_max_size;
/* XXX:make smarter */
