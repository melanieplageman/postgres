							   ...
							   PgAioIPFlags *new_flags)
{
	...
	else
	{
		 ...
		 * parts of the read are in the kernel page cache, but others aren't.
		 * So don't issue WARNING/ERROR, but just retry.
		 *
...
pgaio_process_io_completion(PgAioInProgress *io, int result)
{
	 ...
	 * AIO_CHAOS_{READ,WRITE,FSYNC}_{SHORT,EAGAIN} to x to inject failures
