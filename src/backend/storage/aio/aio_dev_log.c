...
aio_dev_make_consumption_log(int max_length)
{
	PgStreamingReadConsumptionLog *log =
		palloc0(offsetof(PgStreamingReadConsumptionLog, data) +
				sizeof(PgStreamingReadConsumptionLogItem) * max_length);
...
void
aio_dev_log_consumption(PgStreamingReadConsumptionLog *log, instr_time consumption_time, float avg_tput)
{
	PgStreamingReadConsumptionLogItem *entry;

	Assert(log);
	entry = &log->data[log->length];
	entry->consumption_time = consumption_time;
	entry->avg_tput = avg_tput;
	log->length++;
	Assert(log->length <= log->max_length);
}
...
aio_dev_write_consumption_log(PgStreamingReadConsumptionLog *log, FILE *logfile)
{
	...
	fprintf(logfile, "consumption_time,avg_tput\n");

	for (int i = 0; i < log->length; i++)
	{
		PgStreamingReadConsumptionLogItem *entry = &log->data[i];
		fprintf(logfile, "%lf,%lf\n",
				INSTR_TIME_GET_MICROSEC(entry->consumption_time),
				entry->avg_tput);
	}
}
