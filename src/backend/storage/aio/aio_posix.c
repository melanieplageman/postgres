						 ...
						 pgaio_posix_aio_listio_buffer * lb)
{
	...
	{
		/*
		 * Standard POSIX AIO doesn't have scatter/gather.  This IO might
		 * still have been merged from multiple IOs that access adjacent
		 * regions of a file, but only if the memory is also adjacent.
		 */
		Assert(iovcnt == 1);
...
pgaio_posix_aio_closing_fd(int fd)
{
	...
	/*
	 * XXX TODO until we have a good way to iterate over all submittted IOs
	 * (ie that works for retries) looking for the fd, we'll just be
