...
/* GUCs */
int			io_method;
int			max_aio_in_progress = 32768;	/* XXX: Multiple of MaxBackends
											 * instead? */
...
pgaio_postmaster_before_child_exit(int code, Datum arg)
{
	...
	/*
	 * Need to wait for in-progress IOs initiated by this backend to finish.
	 * Some operating systems, like linux w/ io_uring, cancel IOs that are
	 * still in progress when exiting. Other's don't provide access to the
	 * results of such IOs.
	 *
...
pgaio_io_get(void)
{
	...
	io->submitter_id = my_aio_id;	/* XXX or invalid, with defences
									 * elsewhere? */
...
pgaio_io_stage(PgAioInProgress *io, PgAioSharedCallback scb)
{
	...
	if (message_level_is_interesting(DEBUG3))
	{
		...
		/*
		 * This code isn't ever allowed to error out, but the debugging output
		 * is super useful...
		 */
		START_CRIT_SECTION();
...
wait_ref_again:
	...
	while (true)
	{
		...
		if (my_aio->pending_count > 0 && call_shared)
		{
			 ...
			 * requests, to avoid others having to wait for us to submit them.
			 * Don't want to do so when not needing to sleep, as submitting
...
pgaio_io_retry(PgAioInProgress *io)
{
	...
	/*
	 * The IO has completed already, so we need to mark it as in-flight again.
	 */
	Assert(my_aio->outstanding_count > 0);
	dlist_delete_from(&my_aio->outstanding, &io->owner_node);
	my_aio->outstanding_count--;
