...
datadir_fsync_fname(const char *fname, bool isdir, int elevel, uintptr_t state)
{
	...
	if (io_method == IOMETHOD_POSIX_AIO && isdir)
	{
		 ...
		 * non-writable fds should report EBADF).  It also doesn't let you
		 * open directories O_RDWR, so there is no way to synchronize a
		 * directory asynchronously.  Do it synchronously instead.
		 *
		 ...
		 * this down into the aio code?  It can't tell if it has a
		 * directory...
		 */
