...
BitmapHeapScanSetup(BitmapHeapScanState *node)
{
	...
	else
	{
		...
		table_rescan_bm(node->scan, node->tbm, pstate, dsa, prefetch_maximum);
	}
...
BitmapAdjustPrefetchIterator(BitmapHeapScanState *node, BitmapTableScanDesc scan)
{
	...
	if (pstate == NULL)
	{
		TBMSerialIterator *prefetch_iterator = scan->prefetch_iterator;
	...
	{
		TBMSharedIterator *prefetch_iterator = scan->shared_prefetch_iterator;
