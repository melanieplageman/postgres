...
tbm_begin_iterate(TIDBitmap *tbm, dsa_area *dsa, dsa_pointer dsp)
{
	...
	{
		iterator.is_shared = true;
		iterator.shared = tbm_attach_shared_iterate(dsa, dsp);
	}
	else
	{
		iterator.is_shared = false;
...
tbm_end_iterate(TBMIterator *iterator)
{
	...
	if (iterator->is_shared)
		tbm_end_shared_iterate(iterator->shared);
	else
...
tbm_iterate(TBMIterator *iterator, TBMIterateResult *tbmres)
{
	...
	if (iterator->is_shared)
		return tbm_shared_iterate(iterator->shared, tbmres);
	else
		return tbm_private_iterate(iterator->private, tbmres);
}
