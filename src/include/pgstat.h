					  ...
					  double *stddev)
{
	...
	*stddev = sqrt(
	...
	/*
	 * We do not explicitly check for floating point exceptions here. However,
	 * if either calculation returned NAN or INFINITY, we reset both the
	 * standard deviation and mean to NAN.
	 */
	if (isinf(*stddev) || isnan(*stddev) || isinf(*mean) || isnan(*mean))
	{
		*stddev = NAN;
		*mean = NAN;
	}
...
extern void pgstat_wal_update_lsntime_stream(XLogRecPtr lsn,
											 TimestampTz time);
extern void pgstat_wal_refresh_target_freeze_duration(TimestampTz cur_time,
													  XLogRecPtr cur_lsn);
